...
- Real speech with internal pauses has not been tested. On September 26, Cinestudy's official [Podcast/Documentary project](https://cinestudy.org/2026/02/20/edit-this-podcast-documentary/) linked [Renfest 2025 HOST SEGMENTS.mp4](https://drive.google.com/file/d/1yNEXV00DG2dpxdjkZnVxtE5LceVYChhY/view?usp=sharing) (17:31, 6.71 GB, 3840×2160), but Google Drive showed “number of allowed playbacks has been exceeded” and its normal Download action returned an HTTP error. That footage was not downloaded. Do not infer its channel count, sample rate, frame rate, or audible content from Drive metadata. The separate 10-second opener is too short for the requested 1–3-minute speech test.
- A smaller candidate, [Entrevista (Parte 1).webm](https://commons.wikimedia.org/wiki/File:Entrevista_(Parte_1).webm), was downloaded to `C:\Projects\PodCut-TestMedia\Wikimedia\Entrevista (Parte 1).webm` (18,595,269 bytes; outside Git). Wikimedia identifies it as creator-uploaded CC0 interview footage, 75.803 seconds, 1920×1080 VP9/Vorbis. VLC decoded its audio as 44.1 kHz stereo; an extracted 48 kHz, 16-bit stereo WAV lasted 75.700 seconds. PodCut's unchanged Natural, Balanced, and Tight detectors found only a 74.38s–75.70s trailing silence of at least 1.1 seconds, **no internal candidate**. Audible speech quality, frame rate, and Premiere import were not confirmed. This clip is unsuitable for the required internal-cut proof. The temporary extracted WAV is also outside Git at `C:\Projects\PodCut-TestMedia\Wikimedia\interview-audio.wav`.
...
- In the current [SequenceEditor reference](https://developer.adobe.com/premiere-pro/uxp/ppro-reference/classes/sequenceeditor), `createRemoveItemsAction` removes a `TrackItemSelection`, not a time interval. A known internal interval would require constructing retained source ranges with the documented clip source-relative in/out and sequence start/end actions, or proving another supported mechanism. [Sequence.createCloneAction](https://developer.adobe.com/premiere-pro/uxp/ppro-reference/classes/sequence) and [Project.lockedAccess / executeTransaction](https://developer.adobe.com/premiere-pro/uxp/ppro-reference/classes/project) are documented, but have not been exercised for this workflow in Premiere. Apply therefore remains disabled.
